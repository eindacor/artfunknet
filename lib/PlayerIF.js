PlayerIF = function(user) {
	...
	var punishForgedQuest = function(player_interface, forged_item_interface, quest_object) {
		...
		player_item_interface.punishForgeryOwner(player_interface);
	...
	var sellTargets = function(player_interface, sell, donate, unique_targets_found) {
		if (sell || donate) {
            ...
            for (var i=0; i<unique_targets_found.length; i++) {
            	...
            	var player_item_interface = new PlayerItemIF(player_interface, new ItemIF(quest_targets[i]));
	...
	this.getQuestTarget = function(artwork_id) {
		...
		quest_target_item = items.findOne(special_query_unid);
		...
		quest_target_item = items.findOne(regular_query_unid);
