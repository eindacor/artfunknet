PlayerIF = function(user_id) {
	...
	this.getQuickDiscardableItemIds = function() {
	    ...
	    items.find({
	    	...
	    	var permissions = new PlayerItemPermissions(this, new ItemReader(item_object._id));
	...
	this.turnInQuest = function(quest_id, sell, donate) {
		...
		if (this.canTurnInQuest(quest_id)) {
            ...
            this.addFunds("quest", user_id, quest_object.reward.money);
            ...
            if (sell || donate) {
            	//TODO replace with ItemReader array
	            ...
	            for (var i=0; i<quest_targets.length; i++) {
	            	...
	            	var player_item_interface = new PlayerItemIF(this, new ItemReader(quest_targets[i]._id));
	                if (sell) {
	                	this.sell();
	                }
	...
	this.purchaseTicket = function(gallery_id) {
        ...
        var all_shown = items.find({'owner': owner_id, 'status': {$in: ['displayed', 'permanent']}}).fetch();

        for (var i=0; i<all_shown.length; i++) {
        	var player_item_interface = new PlayerItemIF(this, new ItemReader(all_shown[i]._id));
	...
	this.updateGalleryDetails = function() {
		var gallery_value = 0;
	    ...
	    var now = moment()._d.toISOString();
	    var all_displayed = items.find({'owner' : user_id, 'status' : {$in: ['displayed', 'permanent']}}).fetch();

	    for (var i=0; i<all_displayed.length; i++) {
	    	var item_object = all_displayed[i];
	    	var player_item_interface = new PlayerItemIF(this, new ItemReader(item_object._id));
	...
	this.declineAllForSale = function() {
		var all_for_sale = items.find({
	        'owner': user_id,
	        'status': "for_sale", 
	    }).fetch();

		for (var i=0; i<all_for_sale.length; i++) {
			var player_item_interface = new PlayerItemIF(this, new ItemReader(all_for_sale[i]._id));
	...
	this.getSellAllData= function() {
	    ...
	    for (var i=0; i<all_unclaimed.length; i++) {
	    	var item_reader = new ItemReader(all_unclaimed[i]._id);
	    	var permissions = new PlayerItemPermissions(this, item_reader);
	        if (permissions.canQuickDiscard()) {
	            total_value += item_reader.getItemObjectValueByType("sell", user_id);
	...
	this.donateAllUnclaimed = function() {
        ...
        for (var i=0; i<discardable_ids.length; i++) {
            ...
            var player_item_interface = new PlayerItemIF(this, new ItemReader(item_id));
	...
	this.getActiveQuests = function() {
	    return quests.find({'owner_id': user_id}).count();
	}
	...
	this.displayAllTagged = function(tag_array, duration) {
        var tagged_items = items.find({'owner': user_id, 'tags': {$in: tag_array}, 'status': {$ne: 'displayed'}}).fetch();
        var has_capacity = items.find({'owner' : user_id, 'status' : "displayed"}).count() + tagged_items.length <= user_object.profile.display_cap;

        if (!has_capacity)
            return false;

        for (var i=0; i<tagged_items.length; i++) {
            var permissions = new PlayerItemPermissions(this, new ItemReader(tagged_items[i]._id));
            if (!permissions.canDisplay())
                return false;
        }

        for (var i=0; i<tagged_items.length; i++) {
            var player_item_interface = new PlayerItemIF(this, new ItemReader(tagged_items[i]._id));
	...
	this.getDisplayValues = function() {
        ...
        var all_displayed = items.find({'owner' : user_id, 'status' : 'displayed'}).fetch();

        for (var i=0; i<all_displayed.length; i++) {
			var player_item_interface = new PlayerItemIF(this, new ItemReader(all_displayed[i]._id));
	...
	this.getTotalXPPerHour = function() {
		var total_xp = 0;
        var now = moment()._d.toISOString();
        var all_xp_earners = items.find({'owner': user_id, 'status': {$in: ["permanent", "displayed"]}}).fetch();

        for (var i=0; i<all_xp_earners.length; i++) {
        	var player_item_interface = new PlayerItemIF(this, new ItemReader(all_xp_earners[i]._id));
