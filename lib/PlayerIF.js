PlayerIF = function(user) {
	...
	this.increaseVisitorIgnorePenalty = function() {
    	var visitor_ignore_coefficient_increase = .2;
    	var visitor_ignore_proc_count_increase = 10;
    	var visitor_ignore_coefficient_max = .8;

    	var current_ignore_coefficient = user_object.profile.visitor_ignore_coefficient;

    	if (current_ignore_coefficient < visitor_ignore_coefficient_max) {
    		Meteor.users.update({'_id': user_id}, {$inc: {'profile.visitor_ignore_coefficient': visitor_ignore_coefficient_increase, 'profile.visitor_ignore_proc_count': visitor_ignore_proc_count_increase}});
    	}

    	else {
			Meteor.users.update({'_id': user_id}, {$inc: {'profile.visitor_ignore_proc_count': visitor_ignore_proc_count_increase}});
    	}
	}
	...
	this.getForgeryHeat = function(forged_item_object, heat_category) {
	    ...
	    var quality_adjustment_coefficient = 1 - (.6 * forged_item_object.authenticity.forgery_quality);
