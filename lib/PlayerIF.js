PlayerIF = function(user) {
    ...
    user_object = made_from_object ? user : getOneFromCollection("PlayerIF.js:PlayerIF - " + user, Meteor.users, {'_id': user});
	...
	this.registerActivity = function() {
		Meteor.users.update(user_id, {$set: {'profile.last_activity': moment()._d.toISOString()}});
	}

	this.isRecentlyActive = function() {
		var recent_cutoff = ONE_DAY * 30;
		var last_activity = user_object.profile.last_activity;
		var now = moment()._d.toISOString();
		var time_passed = moment(now) - moment(last_activity);
	...
	var punishForgedQuest = function(player_interface, forged_item_interface, quest_object) {
		...
		var rarity_rolled = getRarityDropMapCache()[player_interface.getPlayerLevel()].getRandom();
	...
	this.procUniqueAttribute = function(unique_code, visitor_requirement) {
		try {
	        ...
	        console.log("could not proc " + unique_code + ": " + error);
	        return false;
	    }
	}
	...
	this.vintageProfile = function() {
        ...
        setter["profile.last_activity"] = getNowISOString();
        setter["profile.level"] = 0;
        setter["profile.bank_balance"] = STARTING_BALANCE + Math.floor(getBasicCrateCost(0) * (user_object.profile.vintage_count * 1.5));
        setter["profile.xp"] = 0;
