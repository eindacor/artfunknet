PlayerIF = function(user) {
	...
	this.tutorialMode = function() {
		return TUTORIAL_STATES[Meteor.user().profile.tutorial_data.state] != "finished";
	}
	...
	this.finishTutorials = function() {
		...
		Meteor.users.update(user_id, {$set: {'profile.tutorial_data.state': TUTORIAL_STATES.length - 1, 'profile.tutorial_data.step': 0}});
	}
}