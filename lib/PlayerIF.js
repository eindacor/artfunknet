PlayerIF = function(user) {
	...
	this.addXP = function(xp) {
		...
		if (this.procUniqueAttribute("MONEY_FOR_XP", undefined)) {
			try {
	...
	this.archiveAllUnclaimed = function() {
        ...
        for (var i=0; i<discardable_interfaces.length; i++) {
            ...
            if (!player_item_interface.isRecommendedArchive())
            	continue;

            var archive_categories = item_interface.getArchiveCategories();
            for (var n=0; n<archive_categories.length; n++) {
            	if (player_item_interface.getDisplacedArchiveItem(archive_categories[n]) == undefined) {
            		player_item_interface.archive(archive_categories[n]);
            	}
            }
        }
	}
	...
	this.deleteAllDisplaced = function() {
		items.remove({'owner': user_id, 'status': "archived", 'archive_category': null});
	}
	...
	this.hasDisplacedItems = function() {
		return items.findOne({'owner': user_id, 'status': "archived", 'archive_category': null}) != undefined;
	}
