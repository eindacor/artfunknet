PlayerIF = function(user) {
	...
	this.tutorialMode = function() {
		return TUTORIAL_STATES[user_object.profile.tutorial_data.state] != "finished";
	}
	...
	this.finishTutorials = function() {
		...
		if (snapshot_object) {
			...
			Meteor.users.update(user_id, {$set: {'profile': snapshot_object}});
			this.updateGalleryDetails();
		}

		else {
			Meteor.users.update(user_id, {$set: {'profile.tutorial_data': {'state': TUTORIAL_STATES.indexOf("finished"), 'step': 0}}});
		}
	}
}