		...
		try {
			...
			if (this.canTurnInQuest(quest_id)) {
	            ...
	            getFromCollection("PlayerIF.js:PlayerIF.turnInQuest()", items, {'owner': user_id, 'status': {$nin: ['unclaimed', 'for_sale', 'won']}, 'artwork_id': {$in: quest_object.target}}).forEach(function(item_object) {
	                ...
	                else if (forgery_target_array.indexOf(item_object.artwork_id) && item_object.authenticity.forgery) {
	            ...
	            getFromCollection("PlayerIF.js:PlayerIF.turnInQuest()", items, {'owner': user_id, 'status': {$nin: ['unclaimed', 'for_sale', 'won']}, $and: [{'artwork_id': {$in: forgery_target_array}}, {'artwork_id': {$nin: non_forgery_target_array}}], 'authenticity.forgery': true}).forEach(function(item_object) {
	                if (forgery_found) {
	                	return;
	                }
	            ...
	            getFromCollection("PlayerIF.js:PlayerIF.turnInQuest()", items, {'owner': user_id, 'status': {$nin: ['unclaimed', 'for_sale', 'won']}, 'artwork_id': {$in: quest_object.target}, 'authenticity.forgery': false}).forEach(function(item_object) {
