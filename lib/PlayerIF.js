PlayerIF = function(user) {
	...
	this.getForgeryCost = function(forged_item_object) {
		var values_object = getItemObjectValues(forged_item_object);
		return Math.floor(values_object.actual * FORGERY_COST_COEFFICIENT);
	}

	this.canForgeItem = function(forged_item_object) {
		if (user_object.profile.forgery_contracts == 0) {
			return false;
		}

		return user_object.profile.bank_balance >= this.getForgeryCost(forged_item_object);
	}

	this.punishForgery = function(item_interface) {
		this.chargeAccount(item_interface.getItemObject().authenticity.fee * 2);
	}

	this.forgeItem = function(forged_item_object) {
		if (this.canForgeItem(forged_item_object)) {
			forged_item_object.owner = this.getId();
			this.chargeAccount(this.getForgeryCost(forged_item_object));
			ITEM_GENERATOR.createForgedItem(forged_item_object, function() {
				Meteor.users.update(user_id, {$inc: {'profile.forgery_contracts': -1}});
			});
		}
	}

	this.giveForgeryContracts = function(count) {
		Meteor.users.update(user_id, {$inc: {'profile.forgery_contracts': count}});
	}
	...
	this.getForgeryHeat = function(forged_item_object, heat_category) {
	    ...
	    var quality_adjustment_coefficient = 1 - (.2 * forged_item_object.authenticity.forgery_quality);
