PlayerIF = function(user) {
    ...
    try {
        user_object = made_from_object ? user : getOneFromCollection("PlayerIF.js:ItemIF - " + user, Meteor.users, user);
		...
		try {
			...
			if (this.canTurnInQuest(quest_id)) {
	            ...
	            if (quest_object.reward.item != undefined) {
	                ...
	                var count = getFromCollection("PlayerIF.js:PlayerIF.turnInQuest()", artworks, {'_id': {$nin: loot_data.seasonal_items}, 'rarity': rarity}).count();
	                var random_index = Math.floor(Math.random() * count);
	                var random_artwork_id = getOneFromCollection("PlayerIF.js:PlayerIF.turnInQuest()", artworks, {'_id': {$nin: loot_data.seasonal_items}, 'rarity': rarity}, {skip: random_index})._id;
