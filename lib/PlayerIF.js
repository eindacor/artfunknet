PlayerIF = function(user) {
	...
	this.updateGalleryDetails = function(callback) {
	    ...
	    var visible = TUTORIAL_PLAYER_IDS.indexOf(user_id) != -1 || !this.tutorialMode();
	...
	this.getForgeryCost = function(forged_item_object, forgery_quality) {
		...
		// if only version of the item exists in the player's inventory, increase cost
		if (this.procUniqueAttribute("FORGE_FROM_INVENTORY", undefined)) {
			if (items.find({'owner': this.getId(), 'artwork_id': forged_item_object.artwork_id, 'status': {$in: ["claimed", "reparing", "displayed"]}}).count() == 
				items.find({'owner': this.getId(), 'artwork_id': forged_item_object.artwork_id}).count()) {
				cost *= 1.5;
			}
		}
	...
	this.getForgeableArtworkIds = function() {
		var valid_statuses;
        if (this.procUniqueAttribute("FORGE_FROM_INVENTORY", undefined)) {
        ...
        if (this.procUniqueAttribute("DEALER_ITEM_FORGERY_DISCOUNT", undefined)) {
            valid_statuses.push("for_sale");
        }

        var aggregate_items = items.aggregate([
            {$match: {'owner': Meteor.userId(), 'status': {$in: valid_statuses}}}, 
            {$project: { _id: 0, artist: "$artwork_data.artist", artist_id: "$artwork_data.artist_id", artwork_id: "$artwork_id"} },
            {$sort: {artwork_id : 1} }
        ]);

        var forgeable_artworks = _.uniq(aggregate_items, false, function(agg_object) {return agg_object.artwork_id});
        var forgeable_artwork_id_array = [];
        for (var i=0; i<forgeable_artworks.length; i++) {
            forgeable_artwork_id_array.push(forgeable_artworks[i].artwork_id);
        }

        return forgeable_artwork_id_array;
	}

	this.canForgeItem = function(forged_item_object, forgery_contract_object) {
		var artwork_id = forged_item_object.artwork_id;
		var forgeable_artwork_id_array = this.getForgeableArtworkIds();

		if (forgeable_artwork_id_array.indexOf(artwork_id) == -1) {
			return false;
		}
	...
	this.tutorialMode = function() {
		return user_object.profile.tutorial_data && user_object.profile.tutorial_data.current_tutorial == "artfunkel basics";
	}
	...
	this.beginTutorial = function(tutorial_name) {
		...
		if (tutorial_name == "artfunkel basics")  {
			setter = {};
			...
			setter.profile.tutorial_data = {
				'current_tutorial': tutorial_name == undefined ? "artfunkel basics" : tutorial_name,
			    'step': 0,
			    'completed': pre_tutorial_snapshot.tutorial_data.completed
			}

			setter.profile["pre_tutorial_snapshot"] = JSON.parse(JSON.stringify(pre_tutorial_snapshot));
			setter.profile.bank_balance = 1000000;
			setter.profile.xp = 0;
