PlayerIF = function(user) {
	...
	var getUniqueTargetItems = function(player_interface, quest_object) {
		...
		for (var i=0; i<quest_object.target.length; i++) {
        	var target_item_object = player_interface.getQuestTarget(quest_object.target[i]);
	...
	var giveQuestReward = function(player_interface, quest_object, unique_targets_found) {
		...
		if (quest_object.reward.item != undefined) {
            ...
            var count = getFromCollection("PlayerIF.js:PlayerIF.giveQuestItemReward()", artworks, {'_id': {$nin: getLootData().seasonal_items}, 'rarity': rarity}).count();
            var random_index = Math.floor(Math.random() * count);
            var random_artwork = getOneFromCollection("PlayerIF.js:PlayerIF.giveQuestItemReward()", artworks, {'_id': {$nin: getLootData().seasonal_items}, 'rarity': rarity}, {skip: random_index});
	...
	this.addFunds = function(source, amount) {
	    ...
	    if (source == "auction" && this.tutorialMode()) {
	    	Meteor.users.update(user_id, {$inc: {"profile.pre_tutorial_snapshot.bank_balance" : actual_amount}});
	    }

	    else Meteor.users.update(user_id, {$inc: {"profile.bank_balance" : actual_amount}});
	...
	this.updateGalleryDetails = function(callback) {
	    ...
	    var all_displayed;
	   	if (this.tutorialMode()) {
	    	all_displayed = getFromCollection("PlayerIF.js:PlayerIF.updateGalleryDetails()", items, {'owner' : user_id, 'status' : "displayed", 'tutorial': true}).fetch();
	    }
	    else {
	    ...
	    var visible = !this.tutorialMode();
	    var tutorial = TUTORIAL_PLAYER_IDS.indexOf(user_id) != -1;
	...
	this.getForgeryCost = function(forged_item_object, forgery_quality) {
		...
		if (items.findOne({'owner': this.getId(), 'status': "for_sale", 'artwork_id': forged_item_object.artwork_id}) != undefined && this.procUniqueAttribute("DEALER_ITEM_FORGERY_DISCOUNT", undefined)) {
            cost *= .5;
        }
	...
	this.forgeItem = function(forged_item_object, forgery_contract_id) {
		...
		if (this.canForgeItem(forged_item_object, forgery_contract_object)) {
			...
			forged_item_object.tutorial = this.tutorialMode();
	...
	this.increaseVisitorIgnorePenalty = function(item_interface) {
    	...
    	var visitor_ignore_proc_count_increase = 4;
    	var visitor_ignore_coefficient_max = .8;

    	var rarity_index = artwork_rarities.indexOf(item_interface.getRarity()) + 1;

    	visitor_ignore_proc_count_increase *= rarity_index;
	...
	this.getForgeryHeat = function(forged_item_object, heat_category) {
	    ...
	    if (heat_category == FORGERY_HEAT_CATEGORY.COLLECTOR && this.procUniqueAttribute("COLLECTOR_FORGERY_HEAT_REDUCTION", undefined)) {
	    	heat_coefficient *= .8;
	    }
	...
	this.iterateTutorialState = function(current_state) {
		if (current_state == "finished") {
			return;
		}

		if (TUTORIAL_STATES.indexOf(current_state) == TUTORIAL_STATES.indexOf("finished") - 1) {
			// activate profile
		}

		Meteor.users.update(user_id, {$inc: {'profile.tutorial_data.state': 1}, $set: {'profile.tutorial_data.step': 0}});
	}

	this.changeTutorialStep = function(forward) {
		if (forward)
		{
			var current_state = TUTORIAL_STATES[user_object.profile.tutorial_data.state];
			var current_step =  user_object.profile.tutorial_data.step;

			var max_steps = TUTORIAL_STATE_STEPS[current_state];

			var new_step = current_step == max_steps - 1 ? 0 : current_step + 1;
			Meteor.users.update(user_id, {$set: {'profile.tutorial_data.step': new_step}});

			if (new_step == 0) {
				this.iterateTutorialState(current_state);
			}
		}
			...
			else Meteor.users.update(user_id, {$inc: {'profile.tutorial_data.step': -1}});
		}
	}

	this.changeTutorialState = function(forward) {
		if (forward)
		{
			var current_state = TUTORIAL_STATES[user_object.profile.tutorial_data.state];
			this.iterateTutorialState(current_state);
			...
			else Meteor.users.update(user_id, {$set: {'profile.tutorial_data.step': 0}, $inc: {'profile.tutorial_data.state': -1}});
		}
	}

	this.readyForTutorial = function(tutorial_name) {
		if (user_object.profile.tutorial_data.state == TUTORIAL_STATES.length - 1) {
			return;
		}
		
		var current_state_index = user_object.profile.tutorial_data.state;
		var is_next_state = current_state_index == TUTORIAL_STATES.indexOf(tutorial_name) - 1;

		if (is_next_state) {
			var current_step =  user_object.profile.tutorial_data.step;
			var current_state = TUTORIAL_STATES[current_state_index];
			return current_step == TUTORIAL_STATE_STEPS[current_state] -1
		}

		else return false;
	}

	this.tutorialMode = function() {
		return user_object.profile.tutorial_data && user_object.profile.tutorial_data.state != TUTORIAL_STATES.length - 1;
	}

	this.beginTutorials = function() {
		this.finishTutorials();
		var pre_tutorial_snapshot = Meteor.users.findOne(user_id, {fields: {'profile': 1}}).profile;
		delete pre_tutorial_snapshot["pre_tutorial_snapshot"];

		var setter = {};
		setter.profile = pre_tutorial_snapshot

		setter.profile["pre_tutorial_snapshot"] = JSON.parse(JSON.stringify(pre_tutorial_snapshot));

		setter.profile.tutorial_data = {
			'state': 0,
		    'step': 0
		}

		setter.profile.bank_balance = 100000;
		setter.profile.xp = 0;
		setter.profile.knowledge = {
			'artistic_vision': 0,
			'contextual_understanding': 0,
			'historical_data': 0,
			'technical_comprehension': 0
		}
		setter.profile.last_drop = moment()._d.toISOString();
		setter.profile.last_name_change = moment()._d.toISOString();
		setter.profile.level = 0;
		setter.profile.npcs_met = {
			'bronze': 0,
			'silver': 0,
			'gold': 0,
			'platinum': 0
		}
		setter.profile.vintage_count = 0;
		setter.profile.vintage_select = false;
		setter.profile.visitor_ignore_coefficient = 0;
		setter.profile.visitor_ignore_proc_count = 0;
	...
	this.finishTutorials = function() {
		...
		if (snapshot_object) {
			snapshot_object.tutorial_data.state = TUTORIAL_STATES.length - 1;
			...
			Meteor.users.update(user_id, {$set: {'profile.tutorial_data': {'state': TUTORIAL_STATES.indexOf("finished"), 'step': 0}}}, function() {
		    	var nested_player_interface = new PlayerIF(user_id);
		    	nested_player_interface.updateCaps();
		    	nested_player_interface.refresh();
		    	nested_player_interface.updateGalleryDetails();
	...
	this.getQuestTarget = function(artwork_id) {
		var base_query = {
			'owner': user_id,
	...
	this.getQuestTargetsOwned = function() {
		var targets_owned = {};
		quests.find({'owner_id': user_id}).forEach(function(quest_object) {
			...
			for (var i=0; i<quest_object.target.length; i++) {
				...
				if (targets_owned[artwork_id] == undefined) {
					targets_owned[artwork_id] = nested_player_interface.getQuestTarget(artwork_id);
				}
			}
		});
	...
	this.vintageMode = function() {
		try {
            ...
            var new_bank_balance = STARTING_BALANCE + Math.floor(4000000 * (user_object.profile.vintage_count + 1));
            items.find({'owner': user_id, 'status': {$in: ['for_sale', 'unclaimed', 'won']}}).forEach(function(item_object) {
                removeItem(item_object._id, "vintage clear unclaimed", undefined);
            });

            while (items.findOne({'owner': user_id, 'status': {$in: ['for_sale', 'unclaimed', 'won']}}) != undefined) {
                setTimeout("", 1000);
            }

            var has_items_to_claim = items.findOne({'owner': user_id, 'vintage': {$ne: true}, 'original': {$ne: true}, 'authenticity.forgery': false}) != undefined;
            ...
            items.find({'owner': user_id, $or: [{$and: [{'vintage': true}, {'authenticity.forgery': false}]}, {'original': true}], 'status': {$ne: "archived"}}).forEach(function(item_object) {
                var item_interface = new ItemIF(item_object);
                item_interface.updateItem({$set: {'status': "claimed"}});
            });

            items.find({'owner': user_id, $or: [{'vintage': {$ne: true}}, {'authenticity.forgery': true}], 'original': {$ne: true}, 'status': {$ne: "archived"}}).forEach(function(item_object) {
                var item_interface = new ItemIF(item_object);
