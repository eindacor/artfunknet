PlayerIF = function(user) {
	...
	this.getForgeryCost = function(forged_item_object, forgery_quality) {
		...
		var dealer_discount = this.procUniqueAttribute("DEALER_ITEM_FORGERY_DISCOUNT", undefined) && 
				items.findOne({'owner': this.getId(), 'artwork_id': forged_item_object.artwork_id, 'status': "for_sale"}) != undefined;

		if (!dealer_discount && items.findOne({'owner': this.getId(), 'artwork_id': forged_item_object.artwork_id, 'status': "archived"}) == undefined) {
			cost *= 1.5;
		}
