...
PlayerItemPermissions = function(player_interface, item_interface) {
	...
	this.canSell = function() {
		if (isOwned() && !item_interface.isOriginal()) {
	...
	this.canTagForSale = function() {
		return this.canTag() && item_interface.getItemObject().tags.indexOf("for sale") == -1 && !item_interface.isOriginal();
	}
	...
	var hasInArchive = function() {
		return getOneFromCollection("PlayerItemPermissions.js:hasInArchive", items, {'owner': player_interface.getId(), 'status': "archived", 'archive_category': {$ne: null}, 'artwork_id': item_interface.getArtworkId()}) != undefined;
	}
	...
	this.canArchiveToCategory = function(category) {
		if (!this.canArchive())
			return false;

		if (item_interface.getArchiveCategories().indexOf(category) == -1)
			return false;
	...
	this.canArchive = function() {
		...
		return ["archived", "claimed", "won", "unclaimed"].indexOf(item_interface.getStatus()) != -1;
	}
