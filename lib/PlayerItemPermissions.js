...
PlayerItemPermissions = function(player_interface, item_interface) {
	...
	this.canAuction = function() {
		if (!isPermanent() && isOwnedAndClaimed() && !player_interface.tutorialMode()) {
	...
	this.canSell = function() {
		if (!isPermanent() && isOwned() && !item_interface.isOriginal() && !item_interface.isPermanent() && !player_interface.tutorialMode()) {
	...
	this.canBid = function(amount) {
		if (!player_interface.isActive() || player_interface.tutorialMode())
			return false;
	...
	this.canDonate = function() {
		...
		if (isOwned() && !item_interface.isOriginal() && !isPermanent() && !player_interface.tutorialMode()) {
	...
	this.canArchive = function() {
		...
		return ["archived", "claimed", "won", "unclaimed", "for_sale"].indexOf(item_interface.getStatus()) != -1 && !player_interface.tutorialMode();
	}
