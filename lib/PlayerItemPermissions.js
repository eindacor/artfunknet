...
PlayerItemPermissions = function(player_interface, item_interface) {
	...
	this.canAuction = function() {
		if (!isPermanent() && isOwnedAndClaimed() && !item_interface.isRepairing()) {
	...
	this.canSetPermanent = function() {
		if (isOwned() && !isPermanent() && !artworkIsAlreadyPermanentOrDisplayed() && ["unclaimed", "won", "for_sale"].indexOf(item_interface.getStatus()) == -1 && !item_interface.isDisplaced()) {
	...
	this.canSetRepairing = function() {
		if (isOwned() && !item_interface.isRepairing()) {
	...
	this.canUnsetRepairing = function() {
		return isOwned() && item_interface.isRepairing();
	}
	...
	this.canTagForSale = function() {
		return !isPermanent() && this.canTag() && item_interface.getItemObject().tags.indexOf("for sale") == -1 && !item_interface.isOriginal() && ["auctioned", "archived", "displayed"].indexOf(item_interface.getStatus()) == -1;
	}
	...
	this.canForge = function() {
		if (!isOwned() || forgery_contracts.findOne({'owner_id': player_interface.getId()}) == undefined) {
			return false;
		}
        ...
        return valid_statuses.indexOf(item_interface.getStatus()) != -1;
	}

	this.canIdentify = function() {
		var identify_cost = new PlayerItemIF(player_interface, item_interface).getIdentifyCost();
		return identify_cost <= user_object.profile.bank_balance && isOwnedAndClaimed() && !item_interface.getItemObject().authenticity.identified;
	}
	...
	this.canRedeemForgery = function() {
		...
		return isOwnedAndClaimed() && item_interface.getItemObject().authenticity.liable != player_interface.getId() && item_interface.getItemObject().authenticity.original_owner != player_interface.getId();
	}
}
