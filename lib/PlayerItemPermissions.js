...
PlayerItemPermissions = function(player_interface, item_interface) {
	...
	this.canSetRepairing = function() {
		if (tutorialStatusMatches() && isOwned() && !item_interface.isRepairing()) {
			if (item_interface.getStatus() == "displayed" || item_interface.getStatus() == "auctioned") {
				return false;
			}
	...
	this.canUnsetRepairing = function() {
		if (tutorialStatusMatches() && isOwned() && item_interface.isRepairing()) {
			...
			return item_interface.isRepairing();
		}
	...
	this.canTagForSale = function() {
		return tutorialStatusMatches() && !isPermanent() && this.canTag() && item_interface.getItemObject().tags.indexOf("for sale") == -1 && !item_interface.isOriginal() && item_interface.getStatus() != "auctioned";
	}
	...
	this.canIdentify = function() {
		return tutorialStatusMatches() && isOwnedAndClaimed() && !item_interface.getItemObject().authenticity.identified;
	}
