...
PlayerItemPermissions = function(player_interface, item_interface) {
	...
	var isOwnedAndClaimed = function() {
		var owned = isOwned();
		var permanent = item_interface.isPermanent();
		var claimed_status = item_interface.getStatus() == "claimed";

		return !permanent && owned && claimed_status;
	}
	...
	this.canSetPermanent = function() {
		if (isOwned() && !artworkIsAlreadyPermanentOrDisplayed() && ["unclaimed", "won", "for_sale"].indexOf(item_interface.getStatus()) == -1) {
			return getFromCollection("PlayerItemPermissions.js:PlayerItemPermissions.canSetPermanent()", items, {'permanent': true}).count() < user_object.profile.pc_cap;
		}
	...
	this.canUnsetPermanent = function() {
		return isOwned() && item_interface.isPermanent();
	}
	...
	this.canSell = function() {
		if (isOwned() && !item_interface.isOriginal() && !item_interface.isPermanent()) {
	...
	this.canTagForSale = function() {
		return this.canTag() && item_interface.getItemObject().tags.indexOf("for sale") == -1 && !item_interface.isOriginal() && !item_interface.isPermanent();
	}
	...
	this.canDelete = function() {
		return item_interface.getStatus() == "archived" && !item_interface.isOriginal();
	}
}
