...
PlayerItemPermissions = function(player_interface, item_reader) {
	...
	var isOwned = function() {
		return item_reader.getOwnerId() == user_object._id;
	}
	...
	var artworkIsAlreadyPermanentOrDisplayed = function() {
		return items.findOne({'owner': user_object._id, 'status': {$in: ["displayed", "permanent"]}, 'artwork_id': artwork_object._id}) != undefined;
	}
	...
	this.canUndisplay = function() {
		if (isOwned()) {
			return item_reader.getStatus() == "displayed";
		} 
	}
	...
	this.canQuickDiscard = function() {
		...
		if (!qso.quest_items) {
			...
			var user_does_not_own = items.findOne({'artwork_id': artwork_object._id, 'owner': user_object._id, 'status': {$nin: ["for_sale", "unclaimed", "won", "auctioned"]}}) == undefined;
	...
	this.canAffordUpgrade = function() {
		if (item_reader.getLevel() < MAX_ITEM_LEVEL) {
