...
PlayerItemPermissions = function(player_interface, item_interface) {
	...
	this.canAuction = function() {
		if (tutorialStatusMatches() && !isPermanent() && isOwnedAndClaimed() && !player_interface.tutorialMode()) {
	...
	this.canTagForSale = function() {
		return tutorialStatusMatches() && !isPermanent() && this.canTag() && item_interface.getItemObject().tags.indexOf("for sale") == -1 && !item_interface.isOriginal();
	}
	...
	this.canRedeemForgery = function() {
		...
		return tutorialStatusMatches() && isOwnedAndClaimed() && item_interface.getItemObject().authenticity.liable != player_interface.getId();
	}
}
