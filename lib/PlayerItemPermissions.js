dailyDropIsEnabled = function() {
	if (Meteor.user()) {
		...
		return (moment() - moment(last_drop) > ONE_DAY);
	}
...
PERMISSION_RESPONSES = {
	...
	NOT_ELIGIBLE_FOR_VINTAGE: "this item cannot currently be vintaged",
	...
	NOT_IDENTIFIED: "unidentified items cannot be vintaged"
}
...
PlayerItemPermissions = function(player_interface, item_interface) {
	...
	this.canQuickDiscard = function(to_archive) {
		...
		if (!qso.patreon && item_types.indexOf("patreon") != -1) {
			return negativeResponse(PERMISSION_RESPONSES.QUICK_DISCARD_OPTIONS);
		}
	...
	this.canBid = function(amount) {
		...
		if (auction_object.viewer == "public") {
			var rarity = auction_object.item_data.rarity;
			var level_requirement = getRarityLevelRestrictions()[rarity];
			if (user_object.profile.level < level_requirement) {
	...
	this.canVintage = function() {
		...
		if (["displayed", "claimed"].indexOf(item_interface.getStatus()) == -1) {
			return negativeResponse(PERMISSION_RESPONSES.NOT_ELIGIBLE_FOR_VINTAGE);
		}
...
canMeetNPC = function(npc_id) {
	...
	if (npc_object.expiration && new Date(npc_object.expiration) <= new Date()) {
		return {
			'npc_object': undefined,
			'error': "visitor expired"
		};
	}
