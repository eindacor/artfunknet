...
PlayerItemPermissions = function(player_interface, item_interface) {
	...
	var tutorialStatusMatches = function() {
		return player_interface.tutorialMode() === item_interface.getItemObject().tutorial || player_interface.tutorialMode() && item_interface.getItemObject().tutorial == undefined;
	}
	...
	this.canDisplay = function() {
		if (tutorialStatusMatches() && isOwnedAndClaimed()) {
	...
	this.canUndisplay = function() {
		if (tutorialStatusMatches() && isOwned()) {
			return item_interface.getStatus() == "displayed";
		} 
	}

	this.canAuction = function() {
		if (tutorialStatusMatches() && !isPermanent() && isOwnedAndClaimed() && !player_interface.tutorialMode() && !item_interface.isRepairing()) {
	...
	this.canSetPermanent = function() {
		if (tutorialStatusMatches() && isOwned() && !isPermanent() && !artworkIsAlreadyPermanentOrDisplayed() && ["unclaimed", "won", "for_sale"].indexOf(item_interface.getStatus()) == -1 && !item_interface.isDisplaced()) {
	...
	this.canUnsetPermanent = function() {
		return tutorialStatusMatches() && isOwned() && isPermanent();
	}

	this.canSetRepairing = function() {
		if (tutorialStatusMatches() && isOwned() && !item_interface.isRepairing()) {
			if (["displayed", "auctioned", "archived"].indexOf(item_interface.getStatus()) != -1) {
				return false;
			}

			return getFromCollection("PlayerItemPermissions.js:PlayerItemPermissions.canSetRepairing()", items, {'owner' : user_object._id, 'repairing' : true}).count() < user_object.profile.repairing_cap;
		}
		else return false;
	}

	this.canUnsetRepairing = function() {
		return tutorialStatusMatches() && isOwned() && item_interface.isRepairing();
	}
	...
	var rerollIsEnabled = function() {
		if (tutorialStatusMatches() && isOwned()) {
			...
			if (status == "displayed" || item_interface.isRepairing()) {
				return player_interface.procUniqueAttribute("REROLL_DISPLAY_ENABLE", "Marketing Manager");
			}
	...
	this.canReroll = function() {
		if (tutorialStatusMatches() && rerollIsEnabled()) {
	...
	this.canRerollItemAttribute = function(attribute_id) {
		if (player_interface.tutorialMode() && 
        	attributes.findOne(attribute_id).npc_name != "Art Historian") {
			return false;
		}

		if (tutorialStatusMatches() && this.canReroll()) {
	...
	this.canChangeActiveUniqueAttribute = function() {
		return tutorialStatusMatches() && rerollIsEnabled();
	}

	this.canSell = function() {
		if (tutorialStatusMatches() && !isPermanent() && isOwned() && !item_interface.isOriginal() && !item_interface.isPermanent() && !player_interface.tutorialMode()) {
	...
	this.canDecline = function() {
		if (isOwned() && !player_interface.tutorialMode()) {
	...
	this.canAffordPurchase = function() {
		return tutorialStatusMatches() && getItemObjectValueByType(item_interface.getItemObject(), "dealer", user_object._id) <= user_object.profile.bank_balance;
	}

	this.canTag = function() {
		return tutorialStatusMatches() && isOwned();
	}

	this.canTagForSale = function() {
		return tutorialStatusMatches() && !isPermanent() && this.canTag() && item_interface.getItemObject().tags.indexOf("for sale") == -1 && !item_interface.isOriginal() && ["auctioned", "archived", "displayed"].indexOf(item_interface.getStatus() == -1);
	}

	this.canUntagForSale = function() {
		return tutorialStatusMatches() && this.canTag() && item_interface.getItemObject().tags.indexOf("for sale") != -1;
	}
	...
	this.canQuickDiscard = function(to_archive) {
		if (!tutorialStatusMatches()) {
			return false;
		}

		if (isPermanent() || player_interface.tutorialMode())
			return false;
	...
	this.canBid = function(amount) {
		if (!player_interface.isActive() || player_interface.tutorialMode() || !tutorialStatusMatches())
			return false;
	...
	this.canDonate = function() {
		...
		if (tutorialStatusMatches() && isOwned() && !item_interface.isOriginal() && !isPermanent() && !player_interface.tutorialMode()) {
	...
	this.canAffordUpgrade = function() {
		...
		if (tutorialStatusMatches() && item_interface.getLevel() < MAX_ITEM_LEVEL) {
	...
	this.canUpgrade = function() {
		return tutorialStatusMatches() && rerollIsEnabled() && this.canAffordUpgrade();
	}
	...
	this.canArchive = function() {
		...
		return tutorialStatusMatches() && ["archived", "claimed", "won", "unclaimed", "for_sale"].indexOf(item_interface.getStatus()) != -1 && !player_interface.tutorialMode();
	}

	this.canDelete = function() {
		if (item_interface.isOriginal() || isPermanent() || player_interface.tutorialMode()) {
			return false;
		}

		//TODO why does this work client-side?!
		return item_interface.getStatus() == "archived" || item_interface.isIdentifiedForgery();
	}

	this.canForge = function() {
		return tutorialStatusMatches() && isOwned() && player_interface.getUserObject().profile.forgery_contracts > 0 && item_interface.getStatus() == "archived";
	}

	this.canIdentify = function() {
		var identify_cost = new PlayerItemIF(player_interface, item_interface).getIdentifyCost();
		return identify_cost <= user_object.profile.bank_balance && tutorialStatusMatches() && isOwnedAndClaimed() && !item_interface.getItemObject().authenticity.identified;
	}
	...
	this.canRedeemForgery = function() {
		...
		return tutorialStatusMatches() && isOwnedAndClaimed() && item_interface.getItemObject().authenticity.liable != player_interface.getId() && item_interface.getItemObject().authenticity.original_owner != player_interface.getId();
	}
}
