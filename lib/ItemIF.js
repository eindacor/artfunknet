ItemIF = function(item) {
    ...
    if (typeof item === "string") {
        item_id = item;
        item_object = getOneFromCollection("ItemIF.js:ItemIF - " + item_id, items, item_id);
    }

    // item object was passed
    else {
        item_object = item;
    ...
    this.getRerollCost = function() {
        return item_object.reroll_cost;
    }
    ...
    //TODO add options
    this.updateItem = function(modifier, callback) {
        items.update(item_id, modifier, function(error) {
            if (error)
                ...
                var average_value = Math.floor((rarity_values[item_object.artwork_data.rarity].max + rarity_values[item_object.artwork_data.rarity].min) / 2);
