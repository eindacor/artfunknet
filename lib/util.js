...
setDefaultPhotos = function(){
    ...
    try {
        ...
        console.log("default photo error: " + error);
    }
}
...
getKnowledgeUnitValueFromKnowledgeObject = function(knowledge_object) {
    var total_units = 0;

    for (var i=0; i<knowledge_types.length; i++) {
        var knowledge_type = knowledge_types[i];
        var unit_count = knowledge_object[knowledge_type];
        var converted_units = unit_count * Math.pow(KNOWLEDGE_CONVERSION_BASE, index);

        total_units += converted_units;
    }
...
getArtworkImageURLFromFilename = function(image_name, type, extension) {
    switch(type) {
        case "avatar": return "https://s3.amazonaws.com/com.artfunkel.artwork/avatars/" + image_name + "_avatar." + extension;
        case "thumbnail": return "https://s3.amazonaws.com/com.artfunkel.artwork/thumbnail_images/" + image_name + "_thumb." + extension;
        case "card": return "https://s3.amazonaws.com/com.artfunkel.artwork/card_images/" + image_name + "_card." + extension;
        default: return "https://s3.amazonaws.com/com.artfunkel.artwork/full_images/" + image_name + "." + extension; 
    } 
}

printErrorCallback = function(error) {
    if (error) {
        console.log(error);
    }
}
...
interpolateValues = function(first, second, scale) {
    if (first == second) {
        return first;
    }
    else {
        var delta = Math.max(first, second) - Math.min(first, second);

        if (first > second) {
            return first - (scale * delta);
        }
        else {
            return first + (scale * delta);
        }
