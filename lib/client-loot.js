...
procUniqueAttribute = function(user_id, unique_code, required_npc_name, suppress_notification) {
    try {
        ...
        for (var i=0; i<artworks_found.length; i++) {
            ...
            if (item_object) {
                if (suppress_notification == undefined || Meteor.users.findOne({'_id': user_id, 'profile.notifications.procs.code': unique_code}) == undefined)
                    Meteor.users.update(user_id, {$push: {'profile.notifications.procs': {'expiration': moment().add(5, "seconds")._d.toISOString(), 'artist': item_object.artwork_data.artist, 'title': item_object.artwork_data.title, 'code': unique_code}}});
