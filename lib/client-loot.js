...
procUniqueAttribute = function(user_id, unique_code, required_npc_name, suppress_notification) {
    try {
        ...
        artworks.find({'locked_attributes': {$all: unique_attribute_object.linked_attributes}}).forEach(function(artwork_object) {
            if (item_object != undefined)
                return;

            item_object = items.findOne({'owner': user_id, 'artwork_id': artwork_object._id, 'status': "displayed"});          
        });
