...
procUniqueAttribute = function(user_id, unique_code, required_npc_name, suppress_notification) {
    try {
        ...
        for (var i=0; i<artworks_found.length; i++) {
            ...
            if (item_object) {
                if (suppress_notification == undefined || Meteor.users.findOne({'_id': user_id, 'profile.notifications.code': unique_code}) == undefined)
                    Meteor.users.update(user_id, {$push: {'profile.notifications': {'type': "proc", 'artist': item_object.artwork_data.artist, 'title': item_object.artwork_data.title, 'code': unique_code}}});
