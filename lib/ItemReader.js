ItemReader = function(item_id) {
    ...
    var convertUnitCostToKnowledge = function(unit_cost) {
    ...
    this.getUpgradeCost = function() {
        var total_unit_cost = getUnitValue() * 6;
        return convertUnitCostToKnowledge(total_unit_cost);
    }

    this.getDonationReward = function() { 
        var random_modifier = 1 + ((.5 - Math.random()) * .2);
        var total_unit_cost = Math.floor(getUnitValue() * random_modifier);
        return convertUnitCostToKnowledge(total_unit_cost);
    }
