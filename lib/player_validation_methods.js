...
itemIsOwnedAndClaimed = function(item_id) {
	return itemObjectIsOwnedAndClamed(items.findOne(item_id));
}

itemObjectIsOwnedAndClamed = function(item_object) {
	if (item_object) {
		var owned = Meteor.userId() && item_object.owner == Meteor.userId();
		var claimed_status = item_object.status == "claimed";
...
canDisplayItem = function(item_id) {
	return canDisplayItemObject(items.findOne(item_id))
}

canDisplayItemObject = function(item_object) {
	if (item_object && itemObjectIsOwnedAndClamed(item_object)) {
...
canAuctionItem = function(item_id) {
	return canAuctionItemObject(items.findOne(item_id));
}
...
canSetPermanent = function(item_id) {
	return canSetPermanentItemObject(items.findOne(item_id));
}

canSetPermanentItemObject = function(item_object) {
	if (item_object && itemObjectIsOwnedAndClamed(item_object)) {
...
canUnsetPermanent = function(item_id) {
	return canUnsetPermanentItemObject(items.findOne(item_id));
}

canUnsetPermanentItemObject = function(item_object) {
	if (item_object) {
		var owned = item_object.owner == Meteor.userId();
		var is_permanent = item_object.status == "permanent";
		return is_permanent && owned ? item_object : undefined;
	}
...
canRerollItem = function(item_id) {
	return canRerollItemObject(items.findOne(item_id));
}

canRerollItemObject = function(item_object) {
	if (item_object) {
		var item_owned = Meteor.userId() && item_object.owner == Meteor.userId();
		var can_afford = getRerollCost(item_object._id) <= Meteor.user().profile.bank_balance;
...
canRerollItemAttribute = function(item_id, attribute_id) {
	return canRerollItemObjectAttribute(canRerollItem(item_id), attribute_id);
}

canRerollItemObjectAttribute = function(item_object, attribute_id) {
	if (item_object) {
...
canChangeActiveUniqueAttribute = function(item_id) {
	return canChangeActiveUniqueAttributeItemObject(items.findOne(item_id));
}

canChangeActiveUniqueAttributeItemObject = function(item_object) {
	if (item_object) {
		var item_owned = Meteor.userId() && item_object.owner == Meteor.userId();
		var unique_bypass = procUniqueAttribute(Meteor.userId(), "REROLL_DISPLAY_ENABLE", "Designer");
		var valid_status = item_object.status == "claimed" || unique_bypass;
...
canSellItem = function(item_id) {
	return canSellItemObject(items.findOne(item_id));
}

canSellItemObject = function(item_object) {
	if (item_object) {
		var owned = Meteor.userId() && item_object.owner == Meteor.userId();
		var status_ok = (item_object.status == "claimed" || item_object.status == "unclaimed" || item_object.status == "won");
...
canClaimItem = function(item_id) {
	return canClaimItemObject(items.findOne(item_id));
}

canClaimItemObject = function(item_object) {
	if (item_object) {
		var owned = Meteor.userId() && item_object.owner == Meteor.userId();
		var status_ok = (item_object.status == "unclaimed" || item_object.status == "won");
...
canDeclineItem = function(item_id) {
	return canDeclineItemObject(items.findOne(item_id));
}

canDeclineItemObject = function(item_object) {
	if (item_object) {
		var owned = Meteor.userId() && item_object.owner == Meteor.userId();
		var status_ok = item_object.status == "for_sale";
		return owned && status_ok ? item_object : undefined;
	}
...
canPurchaseItemFromDealer = function(item_id) {
	return canPurchaseItemObjectFromDealer(items.findOne(item_id));
}

canPurchaseItemObjectFromDealer = function(item_object) {
	if (item_object) {
		var owned = Meteor.userId() && item_object.owner == Meteor.userId();
		var status_ok = item_object.status == "for_sale";
