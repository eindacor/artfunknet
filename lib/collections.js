...
if (Meteor.isServer) {
    ...
    Meteor.publish("auctions", function() {
        return auctions.find({'viewer': {$in: [this.userId, "public"]}}, {fields: {'_id': 1, 'min_bid': 1, 'buy_now': 1, 'seller': 1, 'expiration': 1, 'has_bid': 1}});
    });

    Meteor.publish("items", function() {
        // return items.find({});
        return items.find({$or: [
            {'owner': {$in: [this.userId, "Artfunkel, Inc."]}}, 
            {'status': {
                $in: ["auctioned", "displayed", "permanent"]
            }}
        ]});
    });
