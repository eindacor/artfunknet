...
if (Meteor.isServer) {
    ...
    Meteor.publish("items", function() {
        ...
        var user_object = Meteor.users.findOne(this.userId);
        if (user_object && user_object.profile.tutorial_data.state != TUTORIAL_STATES.length - 1) {
            return items.find({'owner': this.userId, 'tutorial': true}, {fields: {
    ...
    Meteor.publish("galleries", function() {
        var user_object = Meteor.users.findOne(this.userId);
        if (user_object && user_object.profile.tutorial_data.state != TUTORIAL_STATES.length - 1) {
            return galleries.find({$or: [{'tutorial': true}, {'owner_id': this.userId}]}, {fields: {'procs': 0}});
        }

        return galleries.find({'tutorial': {$ne: true}}, {fields: {'procs': 0}});
    });
