PlayerItemIF = function(player_interface, item_interface) {
	...
	this.isRecommendedArchive = function() {
		if (item_interface.getStatus() == "archived") {
			return false;
		}

		var displaced_item = this.getDisplacedArchiveItem();
		return displaced_item == undefined || displaced_item.values.actual < item_interface.getItemObject().values.actual;
	}
	...
	this.getDisplacedArchiveItem = function() {
        ...
        var types = item_interface.getTypes();

        for (var i=0; i<types.length; i++) {
            var category = types[i];
        ...
        return getOneFromCollection("ItemIF.js:getDisplacedArchiveItem", items, {'$and': query_array});
	}

	this.archive = function() {
		if (permissions.canArchive()) {
			var displaced_item = this.getDisplacedArchiveItem();
			item_interface.updateItem({$set: {'status': "archived", 'time_archived': moment()._d.toISOString(), 'displaced': false}, $pull: {'tags': {$in: ["displaced"]}}}, true);
