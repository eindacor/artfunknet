PlayerItemIF = function(player_interface, item_interface) {
	...
	this.isRecommendedArchive = function() {
		var potential_categories = item_interface.getArchiveCategories();
		for (var i=0; i<potential_categories.length; i++) {
			var displaced_item = this.getDisplacedArchiveItem(potential_categories[i]);
			if (displaced_item == undefined || displaced_item.values.actual < item_interface.getItemObject().values.actual) {
				return true;
			}
		}
	...
	this.getDisplacedArchiveItem = function(category) {
		try {
			var query = {
				'_id': {$ne: item_id},
				...
				'artwork_id': item_interface.getArtworkObject()._id
			};
			
			return getOneFromCollection("PlayerItemIf.js:getDisplacedArchiveItem", items, query);
		}
	...
	this.archive = function(category) {
		if (permissions.canArchiveToCategory(category)) {
			var displaced_item = this.getDisplacedArchiveItem(category);
			item_interface.updateItem({$set: {'status': "archived", 'time_archived': moment()._d.toISOString(), 'archive_category': category}, $pull: {'tags': {$in: ["displaced"]}}}, true);

			if (displaced_item) {
				var displaced_item_interface = new ItemIF(displaced_item);
				displaced_item_interface.updateItem({$set: {'time_archived': moment()._d.toISOString(), 'archive_category': undefined}, $push: {'tags': "displaced"}}, true)
			}
		}
	}
