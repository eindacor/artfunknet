PlayerItemIF = function(user_id, item_id) {
	...
	this.getXPPerHour = function(xp_earning_time, display_type) {
		try {
			...
			var xp_chunk_percentage = xp_chunk_min + (item_object.level * .1 * delta);
	...
	var getRerollMin = function(attribute_type) {
	    ...
	    var level_scale = item_object.level / MAX_ITEM_LEVEL;
	...
	this.donate = function() {
		if (permissions.canDonate()) {
			...
			var knowledge_object = item_reader.getUpgradeCost();
