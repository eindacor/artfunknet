PlayerItemIF = function(player_interface, item_interface) {
	...
	this.decline = function() {
	    if (permissions.canDecline()) {
	        item_interface.updateItem({$set: {'owner': "Artfunkel, Inc."}});
	...
	this.claim = function() {
		if (permissions.canClaim()) {
		    ...
		    item_interface.updateItem({$set: {'owner': user_id, 'status' : 'claimed', 'date_received': moment()._d.toISOString()}}, function(error) {
	...
	this.auction = function(starting, buy_now, duration) {
		if (permissions.canAuction()) {
	        ...
	        if (errors.length == 0) {
	            item_interface.updateItem({$set: {'status' : 'auctioned'}}, function() {
	...
	this.tag = function(tags) {
		if (permissions.canTag()) {
	        ...
	        item_interface.updateItem({$set: {'tags': lower_case}});
	    }
	}
	...
	this.changeActiveUniqueAttribute = function(unique_attribute_id) {
		if (permissions.canChangeActiveUniqueAttribute()) {
	        ...
	        else item_interface.updateItem({$set: {'active_unique_attribute': unique_attribute_id}});
		}
	}
	...
	this.purchase = function() {
		if (permissions.canPurchase()) {
            ...
            item_interface.updateItem({$set: {'owner': user_id, 'status' : 'claimed', 'date_received': moment()._d.toISOString()}}, callback);

            if (procUniqueAttribute(user_id, "DEALER_PURCHASE_ROLL_COUNT_SET", undefined)) {
                item_interface.updateItem({$set: {'roll_count': -20}});
            }
        }
	}
	...
	this.setPermanentStatus = function(desired_status) {
        ...
        if (desired_status) {
            item_interface.updateItem({$set: {'status' : 'permanent', 'permanent_post' : moment()._d.toISOString()}});
            this.addToChecklist('displayed');
            this.addToChecklist('seen');
        }

        else {
            item_interface.updateItem({$set: {'status' : 'claimed'}, $unset: {'permanent_post' : ""}});
        }
	}
	...
	this.setDisplayStatus = function(desired_status) {
        ...
        if (desired_status) {
            item_interface.updateItem({$set: {'status' : 'displayed', 'time_displayed' : moment()._d.toISOString()}});
            this.addToChecklist('displayed');
            this.addToChecklist('seen');
        }

        else {
            item_interface.updateItem({$set: {'status' : 'claimed'}, $unset: {'time_displayed' : ""}});
        }
	}
	...
	this.placeBid = function(amount) {
	    ...
	    if (amount >= auction_object.buy_now && auction_object.buy_now != -1) {
	    	var auction_object_interface = new ItemIF(auction_object.item_id);
	        auction_object_interface.updateItem({$set: {'status' : 'won', 'owner': user_id, 'tags': [], 'date_received': moment()._d.toISOString()}}, function(error) {
	            ...
	            var nested_auction_object_interface = new ItemIF(auction_object._id);
	            if (auction_object.item_data.condition < .5 && procUniqueAttribute(user_id, "AUCTION_WIN_CONDITION_INCREASE", undefined)) {
	                nested_auction_object_interface.updateItem({$set: {'condition': .9}});
	            }
	...
	this.upgrade = function() {
		if (permissions.canUpgrade()) {
			...
			item_interface.updateItem({$inc: {'level': 1}});
		}
	}
}