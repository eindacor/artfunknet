PlayerItemIF = function(user_id, item_id) {
	...
	this.getPermanentXPPerHour = function(xp_earn_time) {
		try {
			var pc_xp_max_percentage = .1;
		    var pc_xp_increment = .01;

	        var time_displayed = moment(xp_earn_time) - moment(item_object.permanent_post);
	        var permanent_levels = Math.floor(time_displayed / permanent_level_duration);

	        var amplifier = Math.pow(1.1, Math.min(permanent_levels, display_level_cap));
	        var actual_reward = base_money_per_hour * amplifier;
	        var periods_displayed = Math.floor(time_displayed / permanent_xp_frequency);
	        ...
	        var time_til_next_xp = (permanent_xp_frequency * (periods_displayed + 1)) - time_displayed;
	...
	this.getDisplayValuePerHour = function(display_earning_time) {
		try {
		    ...
		    var base_money_per_hour = Math.floor(avg_drop * .02);
	...
	this.undisplay = function() {
		if (permissions.canUndisplay()) {
			updateItem(item_id, {$set: {'status' : 'claimed', 'time_displayed': undefined}});
		}
	}
