PlayerItemIF = function(player_interface, item_interface) {
	...
	this.decline = function() {
	    if (permissions.canDecline()) {
	        this.updateItem({$set: {'owner': "Artfunkel, Inc."}});
	...
	this.claim = function() {
		if (permissions.canClaim()) {
		    ...
		    this.updateItem({$set: {'owner': user_id, 'status' : 'claimed', 'date_received': moment()._d.toISOString()}}, function(error) {
	...
	this.auction = function(starting, buy_now, duration) {
		if (permissions.canAuction()) {
	        ...
	        if (errors.length == 0) {
	            this.updateItem({$set: {'status' : 'auctioned'}}, function() {
	...
	this.tag = function(tags) {
		if (permissions.canTag()) {
	        ...
	        this.updateItem({$set: {'tags': lower_case}});
	    }
	}
	...
	this.changeActiveUniqueAttribute = function(unique_attribute_id) {
		if (permissions.canChangeActiveUniqueAttribute()) {
	        ...
	        else this.updateItem({$set: {'active_unique_attribute': unique_attribute_id}});
		}
	}
	...
	this.purchase = function() {
		if (permissions.canPurchase()) {
            ...
            this.updateItem({$set: {'owner': user_id, 'status' : 'claimed', 'date_received': moment()._d.toISOString()}}, callback);

            if (procUniqueAttribute(user_id, "DEALER_PURCHASE_ROLL_COUNT_SET", undefined)) {
                this.updateItem({$set: {'roll_count': -20}});
            }
        }
	}
	...
	this.setPermanentStatus = function(desired_status) {
        ...
        if (desired_status) {
            this.updateItem({$set: {'status' : 'permanent', 'permanent_post' : moment()._d.toISOString()}});
            this.addToChecklist('displayed');
            this.addToChecklist('seen');
        }

        else {
            this.updateItem({$set: {'status' : 'claimed'}, $unset: {'permanent_post' : ""}});
        }
	}
	...
	this.setDisplayStatus = function(desired_status) {
        ...
        if (desired_status) {
            this.updateItem({$set: {'status' : 'displayed', 'time_displayed' : moment()._d.toISOString()}});
            this.addToChecklist('displayed');
            this.addToChecklist('seen');
        }

        else {
            this.updateItem({$set: {'status' : 'claimed'}, $unset: {'time_displayed' : ""}});
        }
	}
