PlayerItemIF = function(player_interface, item_interface) {
	...
	this.rerollAttributeValue = function(attribute_id) {
	    if (permissions.canReroll(attribute_id)) {
	        ...
	        if (player_interface.tutorialMode() && 
	        	player_interface.getUserObject().profile.tutorial_data.state == TUTORIAL_STATES.indexOf("mod_value") &&
	        	attributes.findOne(attribute_id).npc_name == "Preservationist") {
	        	value = .96;
	        	player_interface.changeTutorialState(true);
	        }

	        else value = getAttributeValue(0, roll_value_min);
	...
	this.rerollAttribute = function(attribute_id) {
        if (permissions.canRerollItemAttribute(attribute_id)) {
			...
			var random_attribute
            var value;
	        if (player_interface.tutorialMode()) {      	
	        	random_attribute = getOneFromCollection("PlayerItemIF.js:PlayerItemIF.rerollAttribute()", attributes, {'npc_name': "Preservationist"});
	        	random_attribute.value = .12;
	        	player_interface.changeTutorialState(true);
	        }

	        else {
	        	random_attribute = getOneFromCollection("PlayerItemIF.js:PlayerItemIF.rerollAttribute()", attributes, selector, {skip: random_index});
	...
	this.alreadyOwnsArtwork = function() {
		...
		if (show_already_owns) {
			...
			var valid_statuses = ['claimed', 'permanent', 'displayed', 'auctioned', 'repairing'];
