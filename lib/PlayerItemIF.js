PlayerItemIF = function(player_interface, item_interface) {
	...
	this.getDisplayValuePerHour = function(display_earning_time) {
        ...
        if (item_interface.isForgery() && !player_interface.procUniqueAttribute("DISPLAY_FORGERY_MONEY_BONUS", undefined)) {
        	console.log("forgery money reward");
        	var forgery_modifier = .5 + (.4 * item_object.authenticity.forgery_quality);
        	actual_reward *= forgery_modifier;
        }
	...
	this.getRerollCost = function() {
		if (item_interface.isForgery() && item_interface.getItemObject().authenticity.identified && player_interface.procUniqueAttribute("FORGERY_REROLL_DISCOUNT", undefined)) {
			return 0;
		}
		    ...
		    if (amount >= auction_object.buy_now && auction_object.buy_now != -1) {
		    	...
		    	var updateCallback = function(error) {
		            ...
		            if (Math.random() < IDENTIFY_FORGED_AUCTIONS_PROC && player_interface.procUniqueAttribute("IDENTIFY_FORGED_AUCTIONS", undefined)) {
		                nested_auction_object_interface.updateItem({$set: {'authenticity.identified': true}})
		            }
		        ...
		        item_interface.updateItem({$set: {
		        	...
		        	'authenticity.identified': user_id == item_interface.getItemObject().authenticity.original_owner,
            		'authenticity.fee': auction_object.buy_now,
	...
	this.redeem = function() {
		try {
			if (permissions.canRedeemForgery()) {
				if (item_interface.getItemObject().authenticity.forgery) {
					var xp_chunk_percentage = .4;
					player_interface.addXPChunkPercentage("redeem forgery", xp_chunk_percentage, false);
					item_interface.punishLiable();
					var money_reward = Math.floor(item_interface.getItemObject().authenticity.fee * FORGERY_REDEMPTION_BONUS_COEFFICIENT);
					player_interface.addFunds("forgery redemption", money_reward);

					var message = "You successfully identified " + item_object.artwork_data.title + " by " + item_object.artwork_data.artist + " as a forgery, earning you XP and $" + getCommaSeparatedValue(money_reward) + ".";
