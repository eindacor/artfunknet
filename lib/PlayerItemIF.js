PlayerItemIF = function(player_interface, item_interface) {
	...
	this.getDisplayValuePerHour = function(display_earning_time) {
	    ...
	    switch(item_object.artwork_data.rarity) {
	        case "common": break;
	        case "uncommon": avg_drop *= 2; break;
	        case "rare": avg_drop *= 4; break;
	        case "legendary": avg_drop *= 7; break;
	        case "masterpiece": avg_drop *= 11; break;
	        default: break;
	    }
	...
	this.decline = function() {
	    if (permissions.canDecline().result) {
	...
	this.claim = function() {
		if (permissions.canClaim().result) {
	...
	this.auction = function(starting, buy_now, duration) {
		...
		if (permissions.canAuction().result) {
			var errors = [];
	        ...
	        if (item_object) {        
	            ...
	            if (Number(starting) < minimum)
	                errors.push("starting value must be at least $" + getCommaSeparatedValue(minimum));

	            if (buy_now != -1 && Number(buy_now) < minimum )
	                errors.push("buy now value must be at least $" + getCommaSeparatedValue(minimum));
	        }
	...
	this.changeActiveUniqueAttribute = function(unique_attribute_id) {
		if (permissions.canChangeActiveUniqueAttribute().result) {
	...
	this.rerollAttributeValue = function(attribute_id) {
	    if (permissions.canReroll(attribute_id).result) {
	...
	this.rerollAttribute = function(attribute_id) {
        if (permissions.canRerollItemAttribute(attribute_id).result) {
            var roll_count = item_object.roll_count;
	...
	this.purchase = function() {
		if (permissions.canPurchase().result) {
	...
	this.setPermanentStatus = function(desired_status) {
		if (desired_status && !permissions.canSetPermanent().result) {
			return false;
		}

		else if (!desired_status && !permissions.canUnsetPermanent().result) {
			return false;
		}
	...
	this.setRepairingStatus = function(desired_status) {
		if (desired_status && !permissions.canSetRepairing().result) {
			return false;
		}

		else if (!desired_status && !permissions.canUnsetRepairing().result) {
			return false;
		}
	...
	this.setDisplayStatus = function(desired_status) {
		if (desired_status && !permissions.canDisplay().result) {
			return false;
		}

		else if (!desired_status && !permissions.canUndisplay().result) {
			return false;
		}
	...
	this.placeBid = function(amount) {
		try {
			...
			if (!permissions.canBid(amount).result)
		        return false;
		    ...
		    if (amount >= auction_object.buy_now && auction_object.buy_now != -1) {
		    	successfulAuction(player_interface, auction_object, auction_object.buy_now);
		    }
	...
	this.getDonationReward = function() {
        ...
        var unit_reward = Math.floor(item_interface.getUnitValue() * random_modifier);

        if (item_interface.isForgery()) {
        	if (player_interface.procUniqueAttribute("DONATE_FORGERY_BONUS", undefined)) {
        		var legendary_multiplier = item_interface.isIdentified() ? 2 : 4;
        		unit_reward = Math.floor(unit_reward * legendary_multiplier);
        	}
        }

        return convertUnitValueToKnowledge(unit_reward);
	}

	this.donate = function() {
		if (permissions.canDonate().result) {
			this.makeLiable();
			...
			if (this.catchForgery(FORGERY_HEAT_CATEGORY.DONATE)) {
				...
				var knowledge_object = this.getDonationReward();
	...
	this.upgrade = function() {
		if (permissions.canUpgrade().result) {
	...
	this.getRecommendedStatus = function() {
		...
		var this_is_archived = item_interface.getStatus() != "archived";
		...
		var is_upgrade = displaced_item ? displaced_item.values.actual < compared_value : false;
		var is_recommended = is_upgrade || (displaced_item == undefined && item_interface.getStatus() != "archived");
	...
	this.archive = function() {
		if (permissions.canArchive().result) {
			this.makeLiable();
	...
	this.delete = function() {
		if (permissions.canDelete().result) {
			items.remove({'_id': item_interface.getId()});
		}
	}
	...
	this.getForgeryHeat = function(heat_category) {
		var plausible_deniability = item_interface.getItemObject().authenticity.identified !== undefined && !item_interface.getItemObject().authenticity.identified;
		return getForgeryHeatFromQuality(item_interface, player_interface, heat_category, item_interface.getItemObject().authenticity.forgery_quality, plausible_deniability);
	}

	this.identify = function() {
		if (permissions.canIdentify().result) {
	...
	this.catchForgery = function(heat_category) {
		if (item_object.authenticity.forgery) {
			var forgery_heat = this.getForgeryHeat(heat_category);
			return Math.random() < forgery_heat;
		}

		else return false;
	}

	this.forge = function(forgery_contract_id) {
		if (permissions.canForge().result) {
			var forgery_contract_object = forgery_contracts.findOne(forgery_contract_id);
			var forgery_quality = forgery_contract_object.quality;
			var cost = player_interface.getForgeryCost(item_interface, forgery_contract_id);
			if (cost > player_interface.getBankBalance()) {
				return false;
			}

			player_interface.chargeAccount(cost);

			ITEM_GENERATOR.forgeItem(item_interface.getId(), forgery_contract_id, function() {
	...
	this.redeem = function() {
		try {
			if (permissions.canRedeemForgery().result) {
				if (item_interface.getItemObject().authenticity.forgery) {
					...
					var html = '<p>You successfully returned <span class="' + item_object.artwork_data.rarity + '" style="font-style:italic">' + item_object.artwork_data.title + '</span> by <span class="af-color">' + item_object.artwork_data.artist + '</span>. You have been refunded <span class="green-text">$' + getCommaSeparatedValue(original_fee) + '</span> for your purchase, and you\'ve earned <span class="green-text">$' + getCommaSeparatedValue(bonus) + '</span> and <span class="af-color">' + getCommaSeparatedValue(xp_earned) + 'xp</span> as a reward.</p>';
		        	alertPlayers(player_interface.getId(), html, 'fa-user-secret', 'good');
	...
	this.makeLiable = function() {
		if (item_object.authenticity.liable == player_interface.getId()) {
			...
			var html = '<p>You have successfully offloaded a forgery (<span class="' + item_object.artwork_data.rarity + '" style="font-style:italic">' + item_object.artwork_data.title + '</span> by <span class="af-color">' + item_object.artwork_data.artist + '</span>), earning you XP.</p>';
        	alertPlayers(former_liable_player_interface.getId(), html, 'fa-user-secret', 'good');
		}	
