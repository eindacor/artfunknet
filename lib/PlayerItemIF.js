PlayerItemIF = function(player_interface, item_interface) {
	...
	this.decline = function() {
	    if (permissions.canDecline()) {
	        item_interface.updateItem({$set: {'owner': "Artfunkel, Inc.", 'status': "auctioned"}}, true);
		...
		if (permissions.canAuction()) {
	        ...
	        if (errors.length == 0) {
	            item_interface.updateItem({$set: {'status' : 'auctioned'}}, true, function() {
	                ...
	                if (user_object.profile.market_expert.expiration > moment()._d.toISOString() && this.procUniqueAttribute("XP_FOR_AUCTIONS", undefined)) {
	...
	this.getRerollMin = function(attribute_type) {
	    ...
	    if (this.procUniqueAttribute("MARKET_EXPERT_ROLL_BONUS", "Auctioneer")) {
	        min_roll += (delta * .3);
	        delta = 1 - min_roll;
	    }
	...
	this.purchase = function() {
		if (permissions.canPurchase()) {
            ...
            if (this.procUniqueAttribute("DEALER_PURCHASE_ROLL_COUNT_SET", undefined)) {
		...
		try {
		    ...
		    if (amount >= auction_object.buy_now && auction_object.buy_now != -1) {
		        ...
		        item_interface.updateItem({$set: {'status' : 'won', 'owner': user_id, 'tags': [], 'date_received': moment()._d.toISOString()}}, false, function(error) {
		            ...
		            if (auction_object.item_data.condition < .5 && this.procUniqueAttribute("AUCTION_WIN_CONDITION_INCREASE", undefined)) {
		                nested_auction_object_interface.updateItem({$set: {'condition': .9}}, true);
		            }

		            if (this.procUniqueAttribute("AUCTION_WIN_TICKET_EXTENSION", undefined)) {
	...
	this.getValue = function(type) {
		try {
            ...
            if (type == "dealer" && this.procUniqueAttribute("DEALER_DISCOUNT", undefined, true)) {
                base_value *= .75;
            }

            if (type == "sell") {
                if (getOneFromCollection("PlayerItemIF.js:PlayerItemIF.getValue()", quests, {'owner_id': user_id, 'target': {$in: [item_object.artwork_id]}}) &&
                    this.procUniqueAttribute("QUEST_ITEM_SELL_BONUS", undefined, true)) {
                    base_value *= 1.5;
                }

                if (item_object.status == "unclaimed" && 
                    this.procUniqueAttribute("UNCLAIMED_ITEM_SELL_BONUS", undefined, true)) {
                    base_value *= 1.5;
                }
            }
