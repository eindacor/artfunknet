PlayerItemIF = function(player_interface, item_interface) {
	...
	this.archive = function(category) {
		if (permissions.canArchive(category)) {
			var displaced_item = this.getDisplacedArchiveItem(category);
			item_interface.updateItem({$set: {'status': "archived", 'time_archived': moment()._d.toISOString(), 'archive_category': category}}, true);

			if (displaced_item) {
				var displaced_item_interface = new item_interface(displaced_item);
				displaced_item_interface.updateItem({$set: {'time_archived': moment()._d.toISOString(), 'archive_category': undefined}}, true)
			}
		}
	}
}