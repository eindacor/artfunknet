	...
	this.catchForgery = function(heat_category) {
		if (item_object.authenticity.forgery) {
			...
			if (item_object.seasonal || item_object.lottery > 0) {
				if (["legendary", "masterpiece"].indexOf(item_object.artwork_data.rarity) == -1) {
					forgery_heat = .99;
				}
			}

			if (forgery_heat == undefined && item_object.unlocked && item_object.artwork_data.rarity == "uncommon") {
				forgery_heat = .99;
			}

			if (forgery_heat == undefined) {
				forgery_heat = player_interface.getForgeryHeat(item_object, heat_category);
			}
