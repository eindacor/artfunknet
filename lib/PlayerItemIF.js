PlayerItemIF = function(player_interface, item_reader) {
	var user_id = player_interface.getId();
	var item_id = item_reader.getId();
	var permissions = new PlayerItemPermissions(player_interface, item_reader);
	...
	this.rerollAttributeValue = function(attribute_id) {
	    if (permissions.canReroll(attribute_id)) {
	        ...
	        player_interface.chargeAccount(item_reader.getRerollCost(item_id));
	...
	this.purchase = function() {
		if (permissions.canPurchase()) {
            player_interface.chargeAccount(item_reader.getItemObjectValueByType("dealer", user_id));
	...
	this.placeBid = function(amount) {
	    ...
	    if (amount >= auction_object.buy_now && auction_object.buy_now != -1) {
	        updateItem(auction_object.item_id, {$set: {'status' : 'won', 'owner': user_id, 'tags': [], 'date_received': moment()._d.toISOString()}}, function(error) {
	            ...
	            var seller_interface = new PlayerIF(seller_id);
