PlayerItemIF = function(player_interface, item_interface) {
	...
	this.redeem = function() {
		if (permissions.canRedeemForgery()) {
			if (item_interface.getItemObject().authenticity.forgery) {
				...
				var liable_player_id = item_interface.getItemObject().authenticity.liable;
				var liable_player_interface = new PlayerIF(liable_player_id);
				liable_player_interface.punishForgery(item_interface);
				items.remove(item_interface.getId());
				player_interface.addFunds("forgery redemption", item_interface.getItemObject().authenticity.fee);
			}

			else {
				player_interface.punishForgery(item_interface);
				items.remove(item_interface.getId());
			}			
		}
	}

	this.forge = function(forged_item_object) {
	...
	this.makeLiable = function() {
		if (item_object.authenticity.liable == player_interface.getId()) {
			...
			item_interface.updateItem({$set: {'authenticity.liable': player_interface.getId(), 'authenticity.liability_pending': true}}, true);
		}		
	}
}