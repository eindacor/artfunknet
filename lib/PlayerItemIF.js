PlayerItemIF = function(user_id, item_id) {
	...
	this.getPermanentLevel = function(xp_earning_time) {
		var time_displayed = moment(xp_earning_time) - moment(item_object.permanent_post);
	    return Math.min(Math.floor(time_displayed / permanent_level_duration), permanent_level_cap);
	}

	this.getPermanentXPPerHour = function(xp_earning_time) {
		try {
			var xp_chunk_min = .01;
			var xp_chunk_max = .04;
			var delta = xp_chunk_max - xp_chunk_min;

			var xp_chunk_percentage = xp_chunk_min + (item_object.xp_rating * delta);
	        
	        var permanent_level = this.getPermanentLevel(xp_earning_time);
	        console.log(permanent_level);
	        

	        var amplifier = Math.pow(1.05, Math.min(permanent_level, permanent_level_cap));
	...
	this.getDisplayValuePerHour = function(display_earning_time) {
		try {
	        ...
	        var amplifier = Math.pow(1.1, display_level);
