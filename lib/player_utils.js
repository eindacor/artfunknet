...
canQuickSell = function(user_object, item_id) {
	...
	if (!qso.quest_items) {
		var item_is_quest_item = quests.findOne({'owner_id': user_object._id, 'target': {$in: [item_wrapper.getArtworkId()]}}) != undefined;
		var user_does_not_own = items.findOne({'artwork_id': item_wrapper.getArtworkId(), 'owner': user_object._id, 'status': {$nin: ["for_sale", "unclaimed", "won", "auctioned"]}}) == undefined;
...
playerHasNotOwned = function(user_object, item_id) {
	...
	var item_types = item_reader.getTypes();
	var item_rarity = item_reader.getRarity();
	var artwork_id = item_reader.getArtworkId();
