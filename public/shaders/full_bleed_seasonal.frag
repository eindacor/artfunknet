...
void main() {
  ...
  float perlinVal = getPerlinValue(uv_coords * .2, .05);

  float transitionVal = pow((1. + sin(u_time * .5)) / 2., 4.);

  vec3 outColor = mix(u_itemRarity, cardImageSample.xyz, pow(perlinVal, .2));
  outColor = mix(cardImageSample.xyz, mix(outColor, vec3(1.) - cardImageSample.xyz, .1), transitionVal);
